...
class MICalculatorVIB(regularizers.Regularizer):
    ...
    def get_mi(self, x):
        ...
        v = 0.5 * (K.sum(cvars, axis=1) + norms -
                   float(k) - K.sum(K.log(cvars), axis=1))
        kl = K.mean(v)
        return kl
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Run Variational IB', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    c_layer = Dense(1024, kernel_initializer='he_uniform',
                    activation='relu')(inputs)
    c_layer = Dense(1024, kernel_initializer='he_uniform',
                    activation='relu')(c_layer)
    c_layer = Dense(512, kernel_initializer='he_uniform',
                    activation='linear', activity_regularizer=micalculator)(c_layer)

    c_layer = noiselayer(c_layer)
    predictions = Dense(trn.nb_classes, init='he_uniform',
                        activation='softmax')(c_layer)
    ...
    adam = keras.optimizers.Adam(lr=0.0001, beta_1=0.5, beta_2=0.999)
    model.compile(loss='categorical_crossentropy',
                  optimizer='adam', metrics=['accuracy'])

    try:
        model.fit(x=trn.X, y=trn.Y, verbose=2, batch_size=100,
                  epochs=200, validation_data=(tst.X, tst.Y), callbacks=cbs)
    ...
    print('Final accuracy (using %d samples): %0.5f' %
          (NUM_SAMPLES, np.mean(preds == tst.y)))
